...
$valLimitOrder = 30;
$sqlMaxOrder = "SELECT   " . $mod_tb_root . "_order  FROM " . $mod_tb_root . " WHERE " . $mod_tb_root . "_masterkey ='" . $_REQUEST['masterkey'] . "'    ";
$sqlMaxOrder .= "  AND " . $mod_tb_root . "_gid ='" . $_REQUEST['inputGh'] . "'  ";
//$sqlMaxOrder .= " AND " . $mod_tb_root . "_typeInfo != 1";
$sqlMaxOrder .= " ORDER BY " . $mod_tb_root . "_order DESC";

// die;
// print_pre($sqlMaxOrder);

$queryMaxOrder = wewebQueryDB($coreLanguageSQL, $sqlMaxOrder);
$rowMaxOrder = wewebFetchArrayDB($coreLanguageSQL, $queryMaxOrder);
$valMaxOrder = $rowMaxOrder[0] + $valLimitOrder;
?>
...
<html xmlns="http://www.w3.org/1999/xhtml">
   ...
   <body>
      <form action="?" method="get" name="myForm" id="myForm">
         ...
         <input name="inputMaxOrder" type="hidden" id="inputMaxOrder" value="<?php echo $valMaxOrder ?>" />
...
" . $mod_tb_root . "_order
  FROM " . $mod_tb_root . " INNER JOIN " . $mod_tb_root_lang . " ON " . $mod_tb_root . "_id = " . $mod_tb_root_lang . "_cid";
                     $sql = $sql . "  WHERE " . $mod_tb_root . "_masterkey ='" . $_REQUEST['masterkey'] . "' AND  " . $mod_tb_root_lang . "_language = 'Thai'";
                     // $sql = $sql." AND ".$mod_tb_root."_typeInfo != 1   ";
                     // print_pre($sql);
...
//                     $sql .= " AND " . $mod_tb_root . "_typeInfo != 1";
// $sql .= " ORDER BY ".$mod_tb_root."_order DESC LIMIT 0,".$valLimitOrder;
                     $sql .= " ORDER BY  " . $mod_tb_root . "_order DESC LIMIT 0," . $valLimitOrder;
// print_pre($sql);
