...
for ($i = 1; $i <= $_REQUEST['TotalCheckBoxID']; $i++) {
   ...
   if (strlen($myVar) >= 1) {
      ...
      $sqlRemore = "SELECT  " . $mod_tb_group_lang . "_pic ," . $mod_tb_group_lang . "_id  FROM " . $mod_tb_group_lang . " WHERE  " . $mod_tb_group_lang . "_cid='" . $permissionID . "' ";
      ...
      ######################### Delete  Contant ###############################
      $sql = "DELETE FROM " . $mod_tb_group_lang . " WHERE " . $mod_tb_group_lang . "_cid=" . $permissionID . " ";
      $Query = wewebQueryDB($coreLanguageSQL, $sql);

      $sql = "DELETE FROM " . $mod_tb_group . " WHERE " . $mod_tb_group . "_id=" . $permissionID . " ";
      $Query = wewebQueryDB($coreLanguageSQL, $sql);
   }
}
