...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadCallape3';
	if(!empty($_FILES['fileToUploadCallape3']['error'])){
		switch($_FILES['fileToUploadCallape3']['error']){
	...
	}elseif($_FILES['fileToUploadCallape3']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$inputGallery=$_FILES['fileToUploadCallape3']['tmp_name'];
			$arrfile=$_FILES['fileToUploadCallape3'];
			$ERROR=$_FILES['fileToUploadCallape3']['error'];
			$TIME=time();
			if(!$ERROR) {
			$filename="pic-callape3-".randomNameUpdate();
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload_mul(\'deletePicAdd-3.php\',\'#boxPicNewCall3\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnameCallape3\" type=\"hidden\" id=\"picnameCallape3\" value=\"$picname\" /> ";
