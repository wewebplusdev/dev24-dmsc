...
<body>
	...
	if (!empty($_FILES['fileToUpload']['error'])) {
		...
		if (!$ERROR) {
			...
			$filename = "picG-" . $_REQUEST['langt'] . "-" . $_REQUEST['myID'] . "-" . randomNameUpdate(2);
			...
			$update = array();
			$update[] = $mod_tb_group_lang . "_pic  	='" . $picname . "'";
			$sql = "UPDATE " . $mod_tb_group_lang . " SET " . implode(",", $update) . " WHERE " . $mod_tb_group_lang . "_id='" . $_REQUEST["myID"] . "' ";
