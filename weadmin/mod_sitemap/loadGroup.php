...
<body>
   ...
   if ($_REQUEST['module_orderby'] == "") {
      $module_orderby = $mod_tb_group . "_order";
   } else {
   ...
   if ($inputSearch <> "") {
      $sqlSearch = $sqlSearch . "  AND  (
		" . $mod_tb_group_lang . "_subject LIKE '%$inputSearch%'  ) ";
   }

   if ($_REQUEST["inputSrchStatus"] != '') {
      $sqlSearch .= " AND " . $mod_tb_group . "_status='" . $_REQUEST["inputSrchStatus"] . "' ";
   }
   ?>
   <form action="?" method="post" name="myForm" id="myForm">
      ...
      <div class="divRightMain">
         <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxListwBorder">
            ...
            $sql .= "INNER JOIN " . $mod_tb_group_lang . " ON " . $mod_tb_group . "." . $mod_tb_group . "_id = " . $mod_tb_group_lang . "." . $mod_tb_group_lang . "_cid";
            $sql = $sql . "  WHERE " . $mod_tb_group . "_masterkey ='" . $_REQUEST['masterkey'] . "'  AND  " . $mod_tb_group_lang . "." . $mod_tb_group_lang . "_language = '" . $_SESSION[$valSiteManage . 'core_session_language'] . "' ";
            $sql = $sql . $sqlSearch;
            ...
            if ($coreLanguageSQL == "mssql") {
               $sql = "SELECT " . $sqlSelect . " FROM (SELECT RuningCount = ROW_NUMBER() OVER (ORDER BY " . $module_orderby . "  " . $module_adesc . " ),*  FROM   " . $mod_tb_group;
               ...
               $sql .= " GROUP BY " . $mod_tb_group . "." . $mod_tb_group . "_id ORDER BY $module_orderby $module_adesc LIMIT $recordstart , $module_pagesize ";
            }
                        ...
                        <?php if ($valPermission == "RW") { ?>
                           <div id="load_status<?php echo $valID ?>">
                              <?php if ($valStatus == "Enable") { ?>
                                 ...
                                 <a href="javascript:void(0)" onclick="changeStatus('load_waiting<?php echo $valID ?>', '<?php echo $mod_tb_group ?>', '<?php echo $valStatus ?>', '<?php echo $valID ?>', 'load_status<?php echo $valID ?>', '../<?php echo $mod_fd_root ?>/statusMg.php')"><span class="<?php echo $valStatusClass ?>"><?php echo $valStatus ?></span></a>
