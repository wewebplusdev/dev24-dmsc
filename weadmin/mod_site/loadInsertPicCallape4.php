...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadCallape4';
	if(!empty($_FILES['fileToUploadCallape4']['error'])){
		switch($_FILES['fileToUploadCallape4']['error']){
	...
	}elseif($_FILES['fileToUploadCallape4']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$inputGallery=$_FILES['fileToUploadCallape4']['tmp_name'];
			$arrfile=$_FILES['fileToUploadCallape4'];
			$ERROR=$_FILES['fileToUploadCallape4']['error'];
			$TIME=time();
			if(!$ERROR) {
			$filename="pic-callape4-".randomNameUpdate();
		...
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload_mul(\'deletePicAdd-4.php\',\'#boxPicNewCall4\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnameCallape4\" type=\"hidden\" id=\"picnameCallape4\" value=\"$picname\" /> ";
