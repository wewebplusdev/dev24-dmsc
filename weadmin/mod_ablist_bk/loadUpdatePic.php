...
<body>
	...
	if (!empty($_FILES['fileToUpload']['error'])) {
		...
		if (!$ERROR) {
			...
			$update = array();
			$update[] = $mod_tb_root . "_icon  	='" . $picname . "'";
			...
			$sqlSch = "UPDATE " . $core_tb_search . " SET " . implode(",", $updateSch) . " WHERE " . $core_tb_search . "_gid='" . $_REQUEST["myID"] . "'  AND " . $core_tb_search . "_masterkey='" . $_REQUEST["masterkey"] . "' ";
			//print_pre($sqlSch);
