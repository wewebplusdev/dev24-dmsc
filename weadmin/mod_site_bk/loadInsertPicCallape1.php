...
<?php
	$error = "";
	$msg = "";
	$fileElementName = 'fileToUploadCallape1';
	if(!empty($_FILES['fileToUploadCallape1']['error'])){
		switch($_FILES['fileToUploadCallape1']['error']){
	...
	}elseif($_FILES['fileToUploadCallape1']['tmp_name'] == 'none'){
		$error = 'No file was uploaded..';
	}else{
		...
		if(!is_dir($mod_path_real)) { mkdir($mod_path_real,0777); }
			...
			$inputGallery=$_FILES['fileToUploadCallape1']['tmp_name'];
			$arrfile=$_FILES['fileToUploadCallape1'];
			$ERROR=$_FILES['fileToUploadCallape1']['error'];
			$TIME=time();
			if(!$ERROR) {
			$filename="pic-callape1-".randomNameUpdate();
		...
		$msg .= "<img src=\"".$mod_path_pictures."/".$picname."\"  style=\"float:left;border:#c8c7cc solid 1px;width: 300px;\"  />";
		$msg .= "<div style=\"width:22px; height:22px;float:left;z-index:1; margin-left:-22px;cursor:pointer;\" onclick=\"delPicUpload_mul(\'deletePicAdd-1.php\',\'#boxPicNewCall1\')\"  title=\"Delete file\" ><img src=\"../img/btn/delete.png\" width=\"22\" height=\"22\"  border=\"0\"/></div>";
		$msg .= "<input name=\"picnameCallape1\" type=\"hidden\" id=\"picnameCallape1\" value=\"$picname\" /> ";
